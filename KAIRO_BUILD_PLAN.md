...
- [x] Custom fields on classes — org-defined fields for reporting/filtering (Region, County, pre-scheduled makeup) (NBC Enhancement) — `sessions.custom_fields JSONB DEFAULT '{}'` + `organizations.custom_field_definitions JSONB DEFAULT '[]'`; custom field values rendered in Reports Schedule tab per class card; TypeScript types updated
- [x] Account age lockout improvement — better error messaging instead of 24-hour lockout; 2-3 attempts before lock (NBC Priority 3) — EmailGate tracks failed lookups in localStorage; 3 failed attempts triggers 15-min lockout with live countdown; attempt-aware copy ("1 attempt remaining before lockout"); amber locked state replaces form; auto-resets on expiry
...
- [x] Waitlist slot available email improvements — clearer email with decline button when spot opens (NBC Enhancement) — `trigger-waitlist-spot-available` edge function sends n8n payload with `waitlist_spot_available_email` intent; includes `registrationUrl` (claim spot) + `declineUrl` (one-click decline); `decline-waitlist-spot` edge function handles GET (email link) and POST (programmatic) — returns styled HTML confirmation page; stamps `notified_at` to prevent duplicates; idempotent on re-decline
